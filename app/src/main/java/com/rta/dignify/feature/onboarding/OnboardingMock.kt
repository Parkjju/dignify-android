 ...
 * 튜토리얼 7장 → 테스트 권유 → 문항 11개 → 결과까지가 확인 대상이라 그 비용이 크다.
 *
 * **끝까지 걸어가면 스스로 꺼진다**([stop]) — 안 그러면 완료를 눌러도 온보딩으로 되돌아와
 * 앱 본편에 못 들어간다. 그래서 앱을 다시 켤 때마다 한 번씩 볼 수 있다.
 *
 * 주의: 서버 호출을 건너뛰지 않는다. 완료를 누르면 **실제로 장르가 저장된다** —
 * 그게 온보딩의 실제 동작이라, 가짜로 만들면 확인하려던 것과 다른 걸 보게 된다.
 * 계정의 장르가 바뀌어도 곤란하지 않은 빌드에서만 켤 것.
 *
