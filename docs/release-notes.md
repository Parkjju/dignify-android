...
기준: `versionCode 12` (1.2.0, 마지막으로 Play에 올라간 빌드).

버그 수정 하나짜리 릴리즈다. 앱 안 노트(`Changelog.kt`)는 **안 붙인다** — 런북 §2대로
버그 수정만이면 새 소식 화면을 띄울 게 없다. 이번엔 유저가 실제로 겪은 문제(온보딩 안내의
"다음"이 실기기에서 안 눌림)라 스토어 노트엔 한 줄 적는다.

### KO

```
• 처음 사용 안내에서 "다음" 버튼이 눌리지 않던 문제를 고쳤어요.
```

### EN (보류 — 등록정보가 KO만이라 지금은 안 올린다)

```
• Fixed the "Next" button not responding in the first-run guide.
```
